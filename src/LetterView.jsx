                    ...
                    users.length > 0 && (
                      <button
                        ...
                        style={{
                          ...
                          borderRadius: 'var(--radius-pill)', padding: '2px 8px', cursor: 'pointer',
                ...
                {c.author_id === currentUser.id && hoveredComment === c.id && editingComment !== c.id && (
                  <div style={{ position: 'absolute', bottom: 8, right: 10, display: 'flex', gap: 10 }}>
                    <button
                      ...
                      style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 11, color: hoveredEdit === c.id ? '#ffffff' : 'var(--text-muted)', fontFamily: 'var(--font-sans)', padding: 0, transition: 'color 0.15s' }}
                    >edit</button>
                    <button
                      onClick={e => { e.stopPropagation(); onDeleteComment(c.id) }}
                      onMouseEnter={() => setHoveredDelete(c.id)} onMouseLeave={() => setHoveredDelete(null)}
                      style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 11, color: hoveredDelete === c.id ? '#ffffff' : '#f87171', fontFamily: 'var(--font-sans)', padding: 0, transition: 'color 0.15s' }}
                    >delete</button>
                  </div>
                )}
              </div>
